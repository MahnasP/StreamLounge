        ...
        `-hls_segment_filename ${outputDir}/segment_%03d.ts`,
          ...
          .on("end", async () => {
              try {
                ...
                const segmentUrls = await Promise.all(segmentFiles.map(async (segment) => {
                ...
                segmentFiles.forEach((segment, index) => {
                    manifestContent = manifestContent.replace(segment, segmentUrls[index]);
                });
                ...
                const manifestUpload = await cloudinary.uploader.upload(manifestPath, { resource_type: "raw" });
